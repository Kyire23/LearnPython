...
def get_grade_list_by_student(userid, action):
    ...
    try:
        if action == "退选":
            ...
            sql = '''SELECT j.JXBID, j.COURSEID, c.COURSENAME, j.USERID, u.USERNAME, j.DESCRIPTION, 0 as SCORE
                   FROM JXB j, Course c, UserInfo u
                  WHERE j.COURSEID = c.COURSEID
                    AND j.USERID = u.USERID
                    AND j.JXBID not in (select JXBID from Grade where USERID = ?);'''            
